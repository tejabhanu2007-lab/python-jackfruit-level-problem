...
class Game:
    ...
    def run(self):
        while True:
            ...
            # --- NEW: check asteroids that passed below the player (lose life) ---
            for a in list(self.asteroids):   # iterate over a snapshot
                # if top of asteroid is below bottom of player, it has passed the player
                if a.rect.top > self.player.rect.bottom:
                    a.kill()                 # remove from groups
                    self.lives -= 1
                    if self.lives <= 0:
                        self.game_over()
            ...
            for bullet in pygame.sprite.groupcollide(self.bullets, self.asteroids, True, True):
                self.score = getattr(self, "score", 0) + 1   # optional score handling
